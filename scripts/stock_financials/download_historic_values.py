...
# def get_historic_data(retrieve_all_query, start, end, timeframe, stock=False):
#     headers = {
#         "APCA-API-KEY-ID": API_KEY,
#         "APCA-API-SECRET-KEY": SECRET_KEY
#     }
...
#     if not stock:
#         stock_info = get_db_info(retrieve_all_query)
#         symbol_lst = stock_info['symbol'].unique().tolist()
#     else:
#         symbol_lst = [stock]
...
#         print(f"[INFO] Finding last data input day for {symbol}")

#         latest_input_query = f"""
#             SELECT MAX(date) AS max_date
#             FROM raw.stock_prices 
#             WHERE symbol = '{symbol}'
#         """

#         df = get_db_info(latest_input_query)
#         latest_date = df["max_date"].iloc[0]    
...
#             params["start"] = get_next_monday(latest_date)
...
#         historic_values = data.get('bars', [])
#         if not historic_values:
#             print(f"[WARNING] No historic data returned for {symbol} between {start} and {end}")
#             return
...
if __name__ == '__main__':
    ...
    fill_stock_values(query, stock=['REMX'])